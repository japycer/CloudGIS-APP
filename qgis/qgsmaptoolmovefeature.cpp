...
QgsMapToolMoveFeature::QgsMapToolMoveFeature( QgsMapCanvas *canvas, MoveMode mode )
  : QgsMapToolAdvancedDigitizing( canvas, DGisApp::instance()->cadDockWidget() )
  ...
  else
  {
    ...
    switch ( mMode )
    {
      ...
      case CopyMove:
        ...
        if ( !DGisApp::instance()->vectorLayerTools()->copyMoveFeatures( vlayer, request, dx, dy, errorMsg, QgsProject::instance()->topologicalEditing(), mSnapIndicator->match().layer() ) )
        {
