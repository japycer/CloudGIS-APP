  ...
  : mMenu( new QMenu )
{
  ...
  mMenu->addAction( DGisApp::instance()->actionShowAllLayers() );
  mMenu->addAction( DGisApp::instance()->actionHideAllLayers() );
  mMenu->addAction( DGisApp::instance()->actionShowSelectedLayers() );
  mMenu->addAction( DGisApp::instance()->actionHideSelectedLayers() );
  mMenu->addAction( DGisApp::instance()->actionToggleSelectedLayers() );
  mMenu->addAction( DGisApp::instance()->actionToggleSelectedLayersIndependently() );
  mMenu->addAction( DGisApp::instance()->actionHideDeselectedLayers() );
  mMenu->addSeparator();
...
QgsMapThemeCollection::MapThemeRecord QgsMapThemes::currentState()
{
  ...
  QgsLayerTreeModel *model = DGisApp::instance()->layerTreeView()->layerTreeModel();
...
void QgsMapThemes::replaceTriggered()
{
  ...
  int res = QMessageBox::question( DGisApp::instance(), tr( "Replace Theme" ),
...
void QgsMapThemes::removeCurrentPreset()
{
  for ( QAction *actionPreset : qgis::as_const( mMenuPresetActions ) )
  {
    if ( actionPreset->isChecked() )
    {
      int res = QMessageBox::question( DGisApp::instance(), tr( "Remove Theme" ),
