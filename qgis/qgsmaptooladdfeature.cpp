...
QgsMapToolAddFeature::QgsMapToolAddFeature( QgsMapCanvas *canvas, CaptureMode mode )
  : QgsMapToolDigitizeFeature( canvas, DGisApp::instance()->cadDockWidget(), mode )
  , mCheckGeometryType( true )
{
  setLayer( canvas->currentLayer() );

  mToolName = tr( "Add feature" );
  connect( DGisApp::instance(), &DGisApp::newProject, this, &QgsMapToolAddFeature::stopCapturing );
  connect( DGisApp::instance(), &DGisApp::projectRead, this, &QgsMapToolAddFeature::stopCapturing );
}
