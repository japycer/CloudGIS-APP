...

{
  ...
  setStyleSheet( DGisApp::instance()->styleSheet() );
  ...
  QgsAttributeEditorContext editorContext = DGisApp::instance()->createAttributeEditorContext();
  ...
  {
    QgsMapCanvas *mc = DGisApp::instance()->mapCanvas();
  ...
  mMainView->init( mLayer, DGisApp::instance()->mapCanvas(), r, editorContext, false );

  QgsAttributeTableConfig config = mLayer->attributeTableConfig();
  mMainView->setAttributeTableConfig( config );

  mFeatureFilterWidget->init( mLayer, editorContext, mMainView, DGisApp::instance()->messageBar(),4 );// DGisApp::instance()->messageTimeout()
  ...
  connect( this, &QgsAttributeTableDialog::saveEdits, this, [ = ] { DGisApp::instance()->saveEdits(); } );//TODO

  const bool dockTable = settings.value( QStringLiteral( "qgis/dockAttributeTable" ), false ).toBool();
  if ( dockTable )
  {
    mDock = new QgsAttributeTableDock( QString(), DGisApp::instance() );
    mDock->setWidget( this );
    connect( this, &QObject::destroyed, mDock, &QWidget::close );
    DGisApp::instance()->addTabifiedDockWidget( Qt::BottomDockWidgetArea, mDock, QStringList(), true );
  }
...
void QgsAttributeTableDialog::keyPressEvent( QKeyEvent *event )
{
  ...
  {
    DGisApp::instance()->deleteSelected( mLayer, this);
  }
}
...
{
  DGisApp::instance()->saveEdits( mLayer, true, true );
}
...
void QgsAttributeTableDialog::mActionExpressionSelect_triggered()
{
  ...
  dlg->setMessageBar( DGisApp::instance()->messageBar() );
...
{
  DGisApp::instance()->cutSelectionToClipboard( mLayer );
}
...
void QgsAttributeTableDialog::mActionCopySelectedRows_triggered()
{
  if ( mMainView->view() == QgsDualView::AttributeTable )
  {
    ...
    DGisApp::instance()->clipboard()->replaceWithCopyOf( featureStore );
  }
  else
  {
    DGisApp::instance()->copySelectionToClipboard( mLayer );
  }
}

void QgsAttributeTableDialog::mActionPasteFeatures_triggered()
{
  DGisApp::instance()->pasteFromClipboard( mLayer );
}


void QgsAttributeTableDialog::mActionZoomMapToSelectedRows_triggered()
{
  DGisApp::instance()->mapCanvas()->zoomToSelected( mLayer );
}

void QgsAttributeTableDialog::mActionPanMapToSelectedRows_triggered()
{
  DGisApp::instance()->mapCanvas()->panToSelected( mLayer );
}
...
void QgsAttributeTableDialog::mActionToggleEditing_toggled( bool )
{
  ...
  if ( !DGisApp::instance()->toggleEditing( mLayer ) )
  {
...
void QgsAttributeTableDialog::mActionRemoveAttribute_triggered()
{
  ...
  if ( dialog.exec() == QDialog::Accepted )
  {
    ...
    {
      DGisApp::instance()->messageBar()->pushMessage( tr( "Attribute error" ), tr( "The attribute(s) could not be deleted" ), Qgis::Warning, DGisApp::instance()->messageTimeout() );
      mLayer->destroyEditCommand();
    }
    // update model - a field has been added or updated
    masterModel->reload( masterModel->index( 0, 0 ), masterModel->index( masterModel->rowCount() - 1, masterModel->columnCount() - 1 ) );
...
void QgsAttributeTableDialog::deleteFeature( const QgsFeatureId fid )
{
  ...
  if ( contextLayers.size() > 1 )
  {
    ...
    DGisApp::instance()->messageBar()->pushMessage( tr( "%1 features deleted: %2" ).arg( deletedCount ).arg( feedbackMessage ), Qgis::Success );
  }
}
...
void QgsAttributeTableDialog::toggleDockMode( bool docked )
{
  ...
  else
  {
    ...
    mDialog = new QDialog( DGisApp::instance(), Qt::Window );
