...
bool QgsGuiVectorLayerTools::startEditing(QgsVectorLayer *layer) const
{
	...
	if (!layer->isEditable() && !layer->readOnly())
	{
		...
		{
			DGisApp::instance()->messageBar()->pushMessage(tr("Start editing failed"),
				tr("Provider cannot be opened for editing"),
				Qgis::Info, 4);//DGisApp::instance()->messageTimeout()
			return false;
		}
...
bool QgsGuiVectorLayerTools::stopEditing(QgsVectorLayer *layer, bool allowCancel) const
{
	...
	if (layer->isModified())
	{
		...
		case QMessageBox::Discard:
			DGisApp::instance()->freezeCanvases();
			if (!layer->rollBack())
			{
				DGisApp::instance()->messageBar()->pushMessage(tr("Error"),
...
void QgsGuiVectorLayerTools::commitError(QgsVectorLayer *vlayer) const
{
	...
	QgsMessageBarItem *errorMsg = new QgsMessageBarItem(
		...
		Qgis::Warning,
		0,
		DGisApp::instance()->messageBar());
	DGisApp::instance()->messageBar()->pushItem(errorMsg);
